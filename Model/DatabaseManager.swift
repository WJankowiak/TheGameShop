...
class DatabaseManager {
    ...
    static func createAndPopulate(){
        ...
        if sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS Games (id INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(16),description VARCHAR(255),  imageName TEXT, price FLOAT, platform TEXT, type TEXT, isInBasket Int)", nil, nil, nil) != SQLITE_OK {
            let errmsg = String (cString: sqlite3_errmsg(db)!)
            print ("error creating table: \(errmsg)")
        }
        for game in gameModelData().gamesData {
            let queryString = "INSERT INTO Games (name, description, imageName, price, platform, type, isInBasket) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    static func getAllGames() -> [Game] {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if(price != 0){
                games.append(Game(name_: name, description_: description, imageName_: imageName, price_: price, platform_: platform, type_: type, isInBasket_: bask))
            }
        }
